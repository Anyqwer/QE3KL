...
{
public:
	SCHEMA_ADD_STRING_OFFSET(m_name, g_offsets::schema_field_name);
	SCHEMA_ADD_OFFSET(uint16_t, m_single_inheritance_offset, g_offsets::schema_field_offset);
	uint8_t m_pad0[0x20];
};
...
{
public:
	SCHEMA_ADD_STRING_OFFSET(m_binary_name, g_offsets::schema_binary_name);

	std::pair<uint16_t, c_schema_class_field_data*> get_fields() const
	{
		return
		{
			m_memory->read_t<uint16_t>(reinterpret_cast<uintptr_t>(this) + g_offsets::schema_fields_count),
			m_memory->read_t<c_schema_class_field_data*>(reinterpret_cast<uintptr_t>(this) + g_offsets::schema_fields_data)
		};
	}
};
...
public:
	...
	{
		auto name = m_memory->read_t<std::string>(reinterpret_cast<uintptr_t>(this) + g_offsets::type_scope_module_name);
		if (name.empty())
			return {};
	...
	{
		return m_memory->read_t<c_utl_ts_hash<c_schema_type_declared_class*, 256, uint32_t>>(reinterpret_cast<uintptr_t>(this) + g_offsets::type_scope_hash_classes);
	}
};
...
public:
	...
	{
		const auto size = m_memory->read_t<uint32_t>(reinterpret_cast<uintptr_t>(this) + g_offsets::schema_type_scopes_size);
		if (!size || size >= 50)
		{
		...
		const auto data = m_memory->read_t<uintptr_t>(reinterpret_cast<uintptr_t>(this) + g_offsets::schema_type_scopes_data);
